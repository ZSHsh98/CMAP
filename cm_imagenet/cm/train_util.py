...
class CMTrainLoop(TrainLoop):
	...
	def run_loop(self):
		...
		while (
			...
			if (self.step % 5000) == 0 & dist.get_rank() == 0:
				generator = get_generator("determ", num_samples=64)
				sample = karras_sample(
					self.diffusion,
					self.model,
					(self.batch_size, 3, self.data.gi_frame.f_locals['image_size'], self.data.gi_frame.f_locals['image_size']),
					steps=40,
					model_kwargs={},
				...
				tvu.save_image((sample + 1) / 2, os.path.join(logger.get_dir(), f'sampled_{self.step}_images.png'))
			...
			if self.global_step % self.log_interval == 0:
				logger.dumpkvs()
				end_time = time.time()
				logger.log(f'time used: {end_time - start_time}')
				start_time = time.time()
