...
sys.path.append('/zhangshuhai/ODEPure/consistency_models')
...
def main():
    ...
    model_and_diffusion_kwargs_t = args_to_dict(
        args, model_and_diffusion_defaults().keys()
    )
    args.class_cond = False
    model_and_diffusion_kwargs_s = args_to_dict(
        args, model_and_diffusion_defaults().keys()
    )
    model_and_diffusion_kwargs_t["distillation"] = distillation
    model_and_diffusion_kwargs_s["distillation"] = distillation
    # model, diffusion = create_model_and_diffusion(**model_and_diffusion_kwargs)
    model, diffusion = create_model_and_diffusion(**model_and_diffusion_kwargs_s)
    ...
    if len(args.teacher_model_path) > 0:  # path to the teacher score model.
        ...
        teacher_model_and_diffusion_kwargs = copy.deepcopy(model_and_diffusion_kwargs_t)
        ...
        teacher_model_parameters = []
        for src in teacher_model.named_parameters():
            if src[0] == 'label_emb.weight':
                pass
            else:
                teacher_model_parameters.append(src[1])

        for dst, i in zip(model.parameters(), range(len(teacher_model_parameters))):
            dst.data.copy_(teacher_model_parameters[i])
    ...
    target_model, _ = create_model_and_diffusion(
        **model_and_diffusion_kwargs_s,
    )
    # target_model.num_classes = None
