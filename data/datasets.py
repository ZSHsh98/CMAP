...
def get_transform(dataset, transform_type, base_size=256):
    ...
    elif "imagenet224" in dataset.lower():
    ...
    elif "imagenet100" in dataset.lower():
        assert base_size == 64, base_size
...
def imagent_dataset_sub(root, transform=None, num_sub=-1, data_seed=0, train=False):
    data_set = torchvision.datasets.ImageFolder(os.path.join(root, 'val'), transform)
    # val_data = torchvision.datasets.ImageFolder(os.path.join(root), transform)
    if train:
         data_set = torchvision.datasets.ImageFolder(os.path.join(root, 'train'), transform)

    if num_sub > 0:
        if not train:
            partition_idx = np.random.RandomState(data_seed).choice(len(data_set), num_sub, replace=False)
        else:
            # partition_idx = np.linspace(0,len(val_data),num_sub, dtype='int')
            partition_idx = np.linspace(0,len(data_set)-1,num_sub, dtype='int')
        data_set = Subset(data_set, partition_idx)
